...
for files_name in os.listdir(zip_path):
	if files_name.endswith(ext):
		...
		for svg_item in svg_array:
			...
			print('bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, svg_item, 'file_search/svg.txt'))
			exp_command = 'bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, svg_item, 'file_search/svg.txt')
			...
			if 'could' not in key:
				...
				pwd_command = 'bkcrack -k %s -r 12 ?a' % key
				...
				if 'Could' not in pwd:
					...
					change_command = 'bkcrack -C %s -k %s -U %s password' % (files_Name, key, new_filename)
		...
		for jpg_item in jpg_array:
			...
			print('bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, svg_item, 'file_search/jpg.txt'))
			exp_command = 'bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, jpg_item, 'file_search/jpg.txt')
			key_lines = subprocess.check_output(exp_command, shell=True).split('\n')
			key = key_lines[-2]
			if 'could' not in key:
				print("Found key %s, attempting to find the password up to 12 char" % key)
		...
		for png_item in png_array:
			...
			print('bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, svg_item, 'file_search/png.txt'))
			exp_command = 'bkcrack -C %s/%s -c %s -p %s' % (zip_path, files_name, png_item, 'file_search/png.txt')
			key_lines = subprocess.check_output(exp_command, shell=True).split('\n')
			key = key_lines[-2]
			if 'could' not in key:
				print("Found key %s, attempting to find the password up to 12 char" % key)
